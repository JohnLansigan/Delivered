<?php
    ...
    $sql = "SELECT u.username, 'account_created' AS event_type, u.dateCreated AS event_time
            ...
            UNION
            SELECT u.username, 'account_updated' AS event_type, u.dateUpdated AS event_time
            FROM tbl_users u
            WHERE u.dateUpdated 
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        switch ($event_type) {
            ...
            case 'admin_permissions':
                $event_text = "[{$username}] Received admin permissions.";
                break;
            case 'message_delivered':
                $event_text = "[{$username}] Delivered a message.";
                break;
            case 'account_updated':
                $event_text = "[{$username}] Updated their account.";
                break;
            default:
